...
public class NavMap extends AppCompatActivity implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        client = getIntent().getExtras().getString("username");
        toolbar = (Toolbar) findViewById(R.id.nav_map_toolbar);
        declineReq = (TextView) findViewById(R.id.decline_req);
        acceptReq = (TextView) findViewById(R.id.accept_req);
        callClient = (ImageView) findViewById(R.id.call_client);
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        prefs = getSharedPreferences("pocketMechanic", MODE_PRIVATE);
        SupportMapFragment mapFrag = (SupportMapFragment) getSupportFragmentManager().findFragmentById(R.id.client_map);
        mapFrag.getMapAsync(NavMap.this);

        acceptReq.setOnClickListener(acceptRequest);
        declineReq.setOnClickListener(declineRequest);
        callClient.setOnClickListener(call);
    }
    ...
    View.OnClickListener declineRequest = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            new SendBackResponse().execute(client, "declined", getIntent().getExtras().getString("id"));
        }
    };

    View.OnClickListener acceptRequest = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            new SendBackResponse().execute(client, "accepted", getIntent().getExtras().getString("id"));
        }
    };
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                if (MainActivity.userLocation != null){
                    new GetAndSendLatLng().execute(client, "both");
                }
            }
        }, 10000);
    }
    ...
    private void setUserMarker(final GoogleMap googleMap) {
        ...
        if (MainActivity.userLocation != null) {
            ...
            marker = googleMap.addMarker(new MarkerOptions().title(getIntent().getExtras().getString("username")).position(userLoc).icon(BitmapDescriptorFactory.fromResource(R.drawable.map_user)));
